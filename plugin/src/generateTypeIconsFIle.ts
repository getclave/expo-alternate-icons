...
/**
 * Generates a TypeScript file containing the `AlternateIcons` type.
...
export function generateTypeIconsFile(iconNames: string[]): void {
    ...
    const typeFilePath = path.resolve(__dirname, '../../src', 'types.ts');
 ...
 */
export type AlternateIcons = ${iconNames.map((name) => `'${name}'`).join(' | ')};
`;
